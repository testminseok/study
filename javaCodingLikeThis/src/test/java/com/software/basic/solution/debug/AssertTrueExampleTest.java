package com.software.basic.solution.debug;
...
public class AssertTrueExampleTest {
    ...
    public void test() {
        ...
        String a = "test";
        String b = "test";
        assertEquals("a 와 b는 동일해야 한다.", a, b);

        // 객체 자체의 동일성을 확인하는 단정 메소드는 따로 있다.
        String c = "test";
        String d = c;
        assertSame("c 와 d는 동일한 객체여야 한다.", c, d);

        // 객체가 null 인지 확인하는 단정 메소드도 따로 있으며
        // assertTrue 는 버그를 발생시킬 가능성이 있다.
        String e = null;
        assertNull("e는 null 이어야 한다.", e);
        assertFalse("b 는 비어있어서는 안된다.", b.isEmpty());
    }
}
